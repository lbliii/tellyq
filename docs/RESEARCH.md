...
| Netflix | Official mobile casting is restricted by device and plan; native Google TV remote can launch deep links | On the Projector Google TV, software opened Netflix, but exact-title testing hit profile error `tvq-prfls-101`; passive Cast status supplied no title or playback state | [Bounded probe](M4-NETFLIX-PROBE.md) blocked at profile entry; compare other services while the app is repaired |
...
software app launch but is blocked by a Netflix profile error. Compare Disney+
