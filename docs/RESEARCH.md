...
| Netflix | Official mobile casting is restricted by device and plan; native Google TV remote can launch deep links | On the Projector Google TV, software opened Netflix; initial exact-title testing hit profile error `tvq-prfls-101`, and a refreshed sign-in retry remained visually unconfirmed. Passive Cast status supplied no title or playback state | [Bounded probe](M4-NETFLIX-PROBE.md) confirms app launch but not an automatable title route; compare other services |
